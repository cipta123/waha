...
export default function Sidebar() {
  ...
  // Hide sidebar on login page (Moved AFTER hooks to prevent hydration error)
  if (pathname === '/login') return null;

  return (
    <>
      {/* DESKTOP SIDEBAR (Hidden on Mobile) */}
      <aside className="hidden md:flex w-16 flex-col items-center gap-6 bg-slate-900 py-6 text-white z-20 relative h-screen">
        {/* Logo */}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 z-50 w-full h-16 bg-slate-900 border-t border-slate-800 flex justify-around items-center text-white">
        <button 
          onClick={() => router.push('/')}
          className={`flex flex-col items-center gap-1 p-2 ${pathname === '/' ? 'text-white' : 'text-white/50'}`}
        >
        ...
        <button 
          onClick={() => router.push('/analytics')}
          className={`flex flex-col items-center gap-1 p-2 ${pathname === '/analytics' ? 'text-white' : 'text-white/50'}`}
        >
          <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
          </svg>
        ...
        <button 
          onClick={() => router.push('/reports')}
          className={`flex flex-col items-center gap-1 p-2 ${pathname === '/reports' ? 'text-white' : 'text-white/50'}`}
        >
          <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
        ...
        <div className="relative flex flex-col items-center gap-1 p-2">
          <button 
            onClick={() => setIsSettingsOpen(!isSettingsOpen)}
            className={`flex flex-col items-center gap-1 ${isSettingsOpen ? 'text-white' : 'text-white/50'}`}
          >
            <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
          ...
          {isSettingsOpen && (
            <div className="absolute bottom-full right-0 mb-2 w-48 rounded-lg bg-white py-1 shadow-xl ring-1 ring-black ring-opacity-5 z-50 overflow-hidden text-slate-900">
                <div className="px-4 py-2 text-xs text-gray-400 font-semibold uppercase">
                  My Account
                </div>
                <button
