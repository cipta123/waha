...
import { fetchRagAnalytics } from '../lib/api';
...
export default function AnalyticsDashboard() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const result = await fetchRagAnalytics(days);
      setData(result);
    } catch (error) {
  ...
  const chatVolumeData = data.daily_chats?.map((item: any) => ({
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500">AI Chatbot performance metrics</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            ...
            <p className="text-2xl font-bold text-gray-900">
                {chatVolumeData.reduce((acc: number, curr: any) => acc + curr.chats, 0)}
            </p>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-gray-500 text-sm font-medium">Avg AI Response</span>
            </div>
            <p className="text-2xl font-bold text-gray-900">
                {data.avg_response_time ? Number(data.avg_response_time).toFixed(2) : '0'} <span className="text-sm text-gray-500 font-normal">sec</span>
            </p>
        </div>
          ...
          <div className="overflow-x-auto">
              <table className="w-full">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                      {topIntents.length > 0 ? (
                          topIntents.map((item: any, index: number) => {
                              const total = topIntents.reduce((acc: number, curr: any) => acc + curr.count, 0);
              ...
              <table className="w-full">
                  <thead className="bg-gray-50">
                      <tr>
                          ...
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Replies Sent</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Performance</th>
                      </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                      {agentPerformance.length > 0 ? (
                          agentPerformance.map((item: any, index: number) => {
                              const maxReplies = Math.max(...agentPerformance.map((a: any) => a.count));
                              const percentage = ((item.count / maxReplies) * 100).toFixed(1);
                              return (
                                  <tr key={index}>
                                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                          <div className="flex items-center gap-2">
                                              <div className="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center text-purple-600 font-bold text-xs">
                                                  {item.agent_id.substring(0, 2).toUpperCase()}
                                              </div>
                                              {item.agent_id}
                                          </div>
                                      </td>
